  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      if (props.data && props.data.imdbID) {
        console.log('Movie ID:', props.data.imdbID);
        try {
...
>>>>>>> 020170b929793e9d0f5fba8cf602ae01a516c900
  ...
  <div>
    ...
    {reviews.map((review) => (
      <div key={review.id} className="d-flex align-items-start mb-3" style={{ color: 'white' }}>
        {review.identities && review.identities.length > 0 ? (
          <img
            src={review.identities[0].identity_data.picture}
            alt={`${review.identities[0].identity_data.full_name}'s Avatar`}
            className="mr-3 rounded-circle"
            style={{ width: '40px', height: '40px' }}
          />
        ) : (
          <div className="mr-3 rounded-circle" style={{ width: '40px', height: '40px', backgroundColor: 'gray' }}></div>
        )}
        <div>
          <p className="mb-1"><strong>{review.identities && review.identities.length > 0 ? review.identities[0].identity_data.full_name : 'Anonymous'}</strong></p>
