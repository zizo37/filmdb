    ...
    if (props.userID) {
      if (!isAddedToList) {
        ...
        try {
          ...
          const { data, error } = await supabase
            .from("watchlist")
            .insert([{ movie_id: props.data.imdbID, user_id: props.userID }]);
          setIsAddedToList(true);
    ...
    <div className="row">
      <div className="col-8">
        <div className="row">
          ...
          >
            {genres.map((item) => (
              <span style={styleElement}> {item}</span>
            ))}
            <p>
              <br></br>
              {props.data.Plot}
      ...
      <div className="col-3" style={parentDivStyle}>
        ...
        >
          <FontAwesomeIcon icon={faPlus} style={plus} />{" "}
          {isAddedToList ? "remove from list watch" : "add to list watch"}
        </button>
